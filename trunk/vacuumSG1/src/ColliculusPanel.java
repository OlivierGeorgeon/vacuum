	...
	public void paintComponent(Graphics g){
		...
		for (int i=0;i<50;i++){
			for (int j=0;j<50;j++){
				...
				// potential map
				//g.setColor(new Color(Math.min(1,colliculus.tmap.potentialMap[i][j]),1-Math.min(1,colliculus.tmap.potentialMap[i][j]),0));
		...
		for (int i=0;i<50;i++){
			for (int j=0;j<50;j++){
				...
				//if (colliculus.vmap.potentialMap[i][j]>0) g.setColor(Color.green);
